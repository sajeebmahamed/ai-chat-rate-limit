...
const permissiveAuthMiddleware = container.get<IPermissiveAuthMiddleware>(
  TYPES.PermissiveAuthMiddleware
);

router.post(
  '/chat',
  (req, res, next) => permissiveAuthMiddleware.authenticate(req, res, next),
